function [ row_tree, col_tree, trials_tree, row_dual_aff_final, col_dual_aff_final, trials_dual_aff_final, col_init_aff, row_init_aff, trials_init_aff ] = RunQuestionnaire3D( params, data )
...
[col_tree, col_eigs(:,1)] = BuildFlexTree(col_init_aff, params.col_tree);

[row_init_aff] = CalcInitAff3D(permute(data, [2 1 3]), params.init_aff_row);
[row_tree, row_eigs(:,1)] = BuildFlexTree(row_init_aff, params.row_tree);

[trials_init_aff] = CalcInitAff3D(permute(data, [1 3 2]), params.init_aff_trials);
[trials_tree, trials_eigs(:,1)] = BuildFlexTree(trials_init_aff, params.trials_tree);
if params.verbose == 2
    figure;
    subplot(3,2,1);
...
if params.init_aff_col.on_rows,
    ...
    for ii = 1:params.n_iters,
        ...
        trials_dual_aff = CalcEmdAff3D(data, col_tree, row_tree, params.row_emd, params.col_emd, params.trials_emd, ~params.init_aff_col.on_rows, ii);
        [trials_tree, trials_eigs(:,ii+1)] = BuildFlexTree(trials_dual_aff, params.trials_tree);
        if params.verbose == 2
            subplot(3,1,3);   
            plotTreeWithColors(trials_tree, 1:length(trials_dual_aff));
            title(['Trials Tree (Iteration ', num2str(ii),')']);
        ...
        row_dual_aff = CalcEmdAff3D(permute(data, [2 3 1]), trials_tree, col_tree, params.col_emd, params.trials_emd, params.col_emd, ~params.init_aff_col.on_rows, ii);
        [row_tree, row_eigs(:,ii+1)] = BuildFlexTree(row_dual_aff, params.row_tree);
        if params.verbose == 2
            subplot(3,1,2);            
            plotTreeWithColors(row_tree, 1:length(row_dual_aff));
            title(['Row Tree (Iteration ', num2str(ii),')']);
        ...
        col_dual_aff = CalcEmdAff3D(permute(data, [1 3 2]), trials_tree, row_tree, params.row_emd, params.trials_emd, params.col_emd, ~params.init_aff_col.on_rows, ii);
        [col_tree, col_eigs(:,ii+1)] = BuildFlexTree(col_dual_aff, params.col_tree);
        if params.verbose == 2
            subplot(3,1,1);
...
%         end
    ...
    trials_dual_aff_final = CalcEmdAff3D(data, col_tree, row_tree, params.row_emd, params.col_emd, params.trials_emd, ~params.init_aff_col.on_rows, ii);
end

row_dual_aff_final = CalcEmdAff3D(permute(data, [2 3 1]), trials_tree, col_tree, params.col_emd, params.trials_emd, params.col_emd, ~params.init_aff_col.on_rows, ii);
col_dual_aff_final = CalcEmdAff3D(permute(data, [1 3 2]), trials_tree, row_tree, params.row_emd, params.trials_emd, params.col_emd, ~params.init_aff_col.on_rows, ii);
if params.verbose == 2
    % plot final affins
    figure;
    subplot(3,1,1);
