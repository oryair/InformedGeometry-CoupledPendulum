function [ aff_mat ] = CalcEmdAffOnTreeLevels( data, row_tree, tree_levels, params)
...
%     row_tree - the resulting partition tree on the rows, given as cell of
%         struct arrays 
...
N = size(data,2);
n_levels = length(row_tree);
emd_mat = zeros(N);

coefs = FindTreeAverages(data, row_tree);

for level = tree_levels,
    W = (row_tree{level}.folder_sizes / sum(row_tree{level}.folder_sizes)) .^ params.beta;
