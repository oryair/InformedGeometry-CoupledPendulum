function [ aff_mat, emd_mat ] = CalcEmdAff3D_mahal( data, col_tree, row_tree, params_row, params_col, params_trials, on_rows, iter )
...
% Average
[coefs, stds]  = FindTreeAveragesAndStds3D(data, col_tree, row_tree );
pb = CmdLineProgressBar('Evaluating EMD with std');
for col_level = 1:col_n_levels,
    for row_level = 1:row_n_levels,
        ...
        for T = 1:size(coefs{row_level, col_level}, 3)
            for n = 1:length(W_row)
                for m = 1:length(W_col)
                    ...
                    s(n, m, T) = stds{row_level, col_level}(n, m, T).^2;
                end
            end
        end
        if any(isnan( w(:)))
        ...
        final_W = zeros(size(w, 3));
        for Ti = 1:size(w, 3)
            for Tj = 1:size(w, 3)
                final_W(Ti, Tj) = sum(sum(((w(:, :, Ti) - w(:, :, Tj)).^2)./(s(:,:,Ti) + s(:,:,Tj))));
            end
        end
...
%         for Ti = 1:size(w, 3)
%             for Tj = 1:size(w, 3)
%                 C(:,Tj+(Ti-1)*size(w, 3)) = reshape(s(:,:,Ti) + s(:,:,Tj), size(s, 1) * size(s, 2), []);
%             end
%         end
